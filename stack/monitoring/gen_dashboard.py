...
panels.append(stat(pid, "Error Rate",
    '(sum(rate(http_requests_total{job="pxtly-api",status=~"4..|5.."}[5m])) or vector(0)) / clamp_min(sum(rate(http_requests_total{job="pxtly-api"}[5m])), 0.001) * 100',
...
panels.append(state_timeline(pid, "Service Availability",
    [
        q('up{job="pxtly-api"}', "Pxtly API", "A"),
...
panels.append(logs_panel(pid, "API Logs",
    '{job="docker", service="pxtly-api"} | json | line_format "[{{.level}}] {{.logger}} {{.message}}"',
    0, y, 12, 12)); pid += 1
