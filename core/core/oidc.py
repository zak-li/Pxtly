...
def _check_audience(payload: dict[str, Any]) -> None:
    """Token must be intended for pxtly-api (aud) or issued by it (azp)."""
...
def extract_role(payload: dict[str, Any]) -> str | None:
    ...
    Checks the custom `pxtly_role` claim first (set via Keycloak attribute mapper),
    then falls back to scanning `realm_access.roles`.
    """
    role = payload.get("pxtly_role")
